...
function PoseCamera() {
  ...
  const getStatusColor=(value)=>{
    if (value == 'معتدل') return "text-green-400";
    if (value == 'منحني') return "text-red-400";
  }
    ...
    <div className="bg-gray-900 p-1 text-center hight-auto flex items-center justify-center">
      <div className="max-w-6xl mx-auto ">
        ...
        <p className="text-center text-xl lg:text-2xl text-gray-400 mb-8">
          منصة دعم التعلم الذكي - تصحيح وضعية الجلوس
        </p>
        {isLoading && (
        ...
        <div className="mt-6 gap-4 flex flex-col lg:flex-row items-center justify-center">
          ...
          <div className=" flex gap-5">
            <div className=" bg-gray-800 p-4 rounded-xl  text-white">
              <h2 className="text-[30px] mb-4">📊 التحليل</h2>
              <p className="text-[20px] text-gray-300 ">
                      الرقبة : <span className={getStatusColor(feedback.neckTilt)}>{feedback.neckTilt}</span>
              </p>
              <p className="text-[20px] text-gray-300 ">الكتفين : <span className={getStatusColor(feedback.shoulders)}>
                          {feedback.shoulders}</span>
              </p>
              <p className="text-[20px] text-gray-300 ">الجذع : <span className={getStatusColor(feedback.torsoTilt)}>
                          {feedback.torsoTilt}</span>
              </p>
            </div>
            <div className=" bg-gray-800 p-4 rounded-xl w-64 text-white ">
              <h2 className="text-[30px] mb-4">🚨 التنبيهات</h2>
                    {alerts.length === 0 && (
                      
              <p className="text-gray-400 text-xl">لا يوجد تنبيهات</p>)}
